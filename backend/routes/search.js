...
// GET /api/search/stations — all unique station names
router.get('/stations', (_req, res) => {
  const names = new Set();
  store.routes.forEach(r => {
    if (Array.isArray(r.stopNames)) r.stopNames.forEach(s => names.add(s));
  });
  // Add legacy STOP_ROUTES keys as well
  Object.keys(STOP_ROUTES).forEach(k => names.add(
